...
export const getBankAccountById = async (id: string): Promise<IResponse<BankAccount.ResponseBankAccount | void>> => {   
    try {
      const existingBankAccount = await bank_account.findOne({ where: { account_number: id } });
      if (!existingBankAccount) {
        return { code: 404, status: "error", details: "BankAccount not found" } as IResponse<void>;
      }

      const bankAccountData = await bank_account.findOne({
      ...
      if (bankAccountData?.closed_date) {
        let obj = {
        ...
        if(bankAccountData.closed_date){
          obj.closed_date = bankAccountData.closed_date;
        }
        //bankAccountData.closed_date?obj.closed_date = bankAccountData.closed_date: null
        return { code: 200, status: "success", description: "Bank Account data retrieved", data: obj } as IResponse<void>;
      }
